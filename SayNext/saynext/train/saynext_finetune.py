...
import transformers
from saynext.dist_utils import init_dist
...
from saynext.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      IMG_CONTEXT_TOKEN, IMG_END_TOKEN,
                                      IMG_START_TOKEN, QUAD_END_TOKEN,
                                      QUAD_START_TOKEN, REF_END_TOKEN,
                                      REF_START_TOKEN, GESTURE_ID)
from saynext.train.dataset import (ConcatDataset, TCSLoader,
                                    WeightedConcatDataset, build_transform,
                                    dynamic_preprocess, preprocess,
                                    preprocess_internlm, preprocess_mpt,
                                    preprocess_phi3)
from saynext.train.trainer_monkey_patch import replace_create_optimizer
...
try:
    ...
    print('petrel_client is not installed. Using PIL to load images from local.')
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        min_dynamic_patch=1,
        max_dynamic_patch=6,
        min_num_frame=2,  # for video data
        max_num_frame=4,  # for video data
        sampling_method='rand',  # for video data
        sampling_interval = 8, #for video data
        repeat_time=1,
    ...
    def _process_video(self, video_path, max_frames, sampling_interval):
        ...
        max_start_index = max(0, total_frames - (max_frames - 1) * sampling_interval)

        if max_start_index <= 0:
            print("Video too short, extracting the last available frames.")
...
def build_eval_datasets(
    ...
    datasets = []
    ds_collections = json.loads(open(data_args.eval_data_path).read())
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        dataset = LazySupervisedDataset(
            data_args.conv_style, ds_collections[ds_name],
            tokenizer,
            ...
            num_image_token=model.num_image_token,
            image_size=data_args.force_image_size,
            is_train=False,  # Ensure it's evaluation mode
            pad2square=data_args.pad2square,
            group_by_length=group_by_length,
            dynamic_image_size=dynamic_image_size,
            use_thumbnail=use_thumbnail,
            ...
            repeat_time=1,  # No repetition for eval datasets
            normalize_type=normalize_type,
            random_seed=ds_idx,
        )
